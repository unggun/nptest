 ...
 * Last Modified: 2019-03-05T10:55:23+00:00
 * File:          app/code/Xtento/OrderImport/Setup/RecurringData.php
...
class RecurringData implements InstallDataInterface
{
    ...
    private function convertSerializedDataToJson(\Magento\Framework\Setup\ModuleDataSetupInterface $setup)
    {
        ...
        $fieldDataConverter = $fieldDataConverterFactory->create('\Xtento\OrderImport\Test\SerializedToJsonDataConverter');
        $fieldsToConvert = ['conditions_serialized'];
        foreach ($fieldsToConvert as $fieldName) {
            $fieldDataConverter->convert(
                $setup->getConnection(),
                $setup->getTable('xtento_orderimport_profile'),
                'profile_id',
                $fieldName
            );
        }
